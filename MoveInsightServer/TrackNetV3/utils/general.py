...
def write_pred_video(video_file, pred_dict, save_file, traj_len=8, label_df=None):
    ...
    while True:
        ...
        pred_queue.appendleft([x_pred[i], y_pred[i]]) if i < len(x_pred) and vis_pred[i] else pred_queue.appendleft(None)
    ...
    # Fix video rotation if needed using ffmpeg
    try:
        import subprocess
        
        # Check if video needs rotation correction using ffprobe
        probe_cmd = [
            'ffprobe', '-v', 'quiet', '-select_streams', 'v:0', 
            '-show_entries', 'stream_tags=rotate', '-of', 'csv=p=0', video_file
        ]
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            if rotation in ['90', '180', '270']:
                print(f"Detected rotation: {rotation} degrees, correcting...")
                temp_file = save_file.replace('.mp4', '_temp.mp4')
                os.rename(save_file, temp_file)
                ...
                fix_cmd = [
                    'ffmpeg', '-i', temp_file, '-vf', transpose, 
                    '-c:a', 'copy', '-y', save_file
                ]
                subprocess.run(fix_cmd, capture_output=True, check=True)
                os.remove(temp_file)
                print("Video rotation corrected")
                
    except (ImportError, subprocess.CalledProcessError, FileNotFoundError):
        # If ffmpeg/ffprobe not available or fails, keep original video
        pass
